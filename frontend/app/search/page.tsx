...
const groupLabels: Record<string, string> = { projects: "Projects", people: "People", requirements: "Requirements", features: "Features", modules: "Modules", apis: "APIs", tasks: "Tasks", reports: "Reports", risks: "Risks" };
...
function SearchResultLink({ item }: { item: SearchItem }) { const href = item.kind === "project" ? `/project/${item.id}` : item.kind === "person" ? `/people/${item.id}` : item.kind === "report" ? `/project/${item.project_id}/reports/${item.id}` : `/project/${item.project_id}/${item.kind === "api" ? "apis" : item.kind === "risk" ? "risks" : item.kind === "task" ? "tasks" : item.kind === "module" ? "structure" : item.kind === "requirement" ? "requirements" : "requirements"}`; return <Link href={href} className="focus-ring rounded-xl border border-[var(--line)] p-4 hover:bg-[var(--peach)]"><p className="font-semibold">{item.title}</p><p className="muted mt-1 line-clamp-2 text-sm">{item.subtitle}</p></Link>; }
