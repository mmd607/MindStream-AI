...
export default function ArchitecturePage() {
  ...
  const component = data?.components_json.find(item => item.name === selected) ?? data?.components_json[0];
  const selectedName = selected || component?.name || "Project core";
  const selectedFeatures = useMemo(() => features.filter(item => item.module_name === selectedName), [features, selectedName]);
  const selectedApis = useMemo(() => apis.filter(item => item.module === selectedName), [apis, selectedName]);
  const selectedTasks = useMemo(() => tasks.filter(item => item.module_name === selectedName), [tasks, selectedName]);
  ...
  return <div className="py-10">
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_.38fr]">
      <Card title={mode === "2d" ? "System graph" : "Spatial structure"} action={mode === "3d" ? <div className="flex items-center gap-1"><button aria-label="Zoom out" title="Zoom out" onClick={() => zoom(-.1)} className="icon-button px-2 py-1">−</button><button aria-label="Reset view" title="Reset view" onClick={resetView} className="icon-button px-2 py-1 text-xs">Reset</button><button aria-label="Zoom in" title="Zoom in" onClick={() => zoom(.1)} className="icon-button px-2 py-1">+</button></div> : <Badge>{data.components_json.length} modules</Badge>}>
        {mode === "2d" ? <TwoDGraph data={data} selected={selectedName} onSelect={setSelected} /> : <ThreeDGraph data={data} selected={selectedName} onSelect={setSelected} scale={scale} offset={offset} dragging={Boolean(drag)} onPointerDown={event => setDrag({ start: { x: event.clientX, y: event.clientY }, origin: offset })} onPointerMove={move} onPointerUp={() => setDrag(null)} onPointerLeave={() => setDrag(null)} />}
      </Card>
...
function TwoDGraph({ data, selected, onSelect }: { data: Architecture; selected: string; onSelect: (name: string) => void }) { return <div className="relative min-h-[440px] overflow-hidden rounded-2xl bg-[var(--peach)] p-5"><div className="mx-auto flex max-w-2xl flex-col items-center gap-4 py-4"><Node label="Web client" symbol="01" /><span className="text-2xl text-[var(--accent)]">↓</span><Node label="API layer" symbol="02" /><span className="text-2xl text-[var(--accent)]">↓</span><div className="grid w-full gap-3 sm:grid-cols-2">{data.components_json.map((item, index) => <button key={item.name} onClick={() => onSelect(item.name)} className={`focus-ring rounded-2xl border-2 bg-[var(--panel)] p-4 text-left shadow-sm transition hover:-translate-y-1 ${selected === item.name ? "border-[var(--accent)]" : "border-[var(--line)]"}`}><span className="eyebrow">Module 0{index + 1}</span><span className="mt-2 block font-bold">{item.name}</span><span className="muted mt-1 block text-xs">{item.responsibility}</span></button>)}</div><span className="text-2xl text-[var(--accent)]">↓</span><Node label="Relational database" symbol="DB" /></div><p className="absolute bottom-4 left-5 text-xs font-semibold text-[var(--muted)]">Select a module to inspect ownership and connected delivery data.</p></div>; }

function ThreeDGraph({ data, selected, onSelect, scale, offset, dragging, onPointerDown, onPointerMove, onPointerUp, onPointerLeave }: { data: Architecture; selected: string; onSelect: (name: string) => void; scale: number; offset: Point; dragging: boolean; onPointerDown: (event: React.PointerEvent<HTMLDivElement>) => void; onPointerMove: (event: React.PointerEvent<HTMLDivElement>) => void; onPointerUp: () => void; onPointerLeave: () => void }) {
  const positions = data.components_json.map((item, index) => { const angle = (index / Math.max(data.components_json.length, 1)) * Math.PI * 2 - Math.PI / 2; return { item, x: Math.cos(angle) * 245, y: Math.sin(angle) * 145, z: index * 8 }; });
